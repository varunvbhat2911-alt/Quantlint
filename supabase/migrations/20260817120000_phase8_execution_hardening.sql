-- Phase 8: execution hardening functions.
--
-- No schema changes (no new columns, no new tables, no altered constraints).
-- Three SECURITY INVOKER functions provide the atomicity that PostgREST
-- cannot: multi-table transactional inserts, atomic stale recovery, and
-- atomic retry reset. All run via the service-role admin client (.rpc()),
-- which bypasses RLS; callers have already verified audit ownership through
-- the RLS-scoped session client before invoking these.

-- ============================================================
-- 1. commit_audit_results — atomic result persistence
--
-- Replaces the previous Promise.all of four independent child inserts +
-- a separate status update. Every insert + the final status/progress
-- update succeed together or roll back together: a failed audit never
-- carries a half-persisted report.
-- ============================================================
...
--
-- Finds audits stuck in 'running' past the staleness threshold and
-- atomically transitions them to 'failed' with a timeline entry.
-- Uses the existing updated_at/moddatetime heartbeat. Concurrency-safe:
-- the UPDATE ... WHERE status='running' is atomic, so two concurrent
-- calls cannot both claim the same audit.
-- ============================================================
...
--
-- Deletes all previous child results and resets the audit to 'queued'
-- in one transaction. Children are deleted while the audit is still
-- 'failed' (no concurrent claimAudit can match 'failed'), then the
-- status transitions to 'queued'. The audit UUID is preserved.
-- Concurrency-safe: the final UPDATE ... WHERE status='failed' ensures
-- only one concurrent retry request succeeds.
-- ============================================================
