...
async function audit(code: string, categories: readonly string[] = ALL_CATEGORIES) {
...
function findingsFor(result: Awaited<ReturnType<typeof audit>>, ruleId: string) {
  return result.findings.filter((f) => f.ruleId === ruleId);
}

describe("QL-BIAS-001 look-ahead bias", () => {
  it("flags negative shift (positive case)", async () => {
    const result = await audit("import pandas as pd\nsignal = close.shift(-1) > mean\n");
  ...
  it("does not flag causal shift(1) (negative case)", async () => {
    const result = await audit("import pandas as pd\nsignal = close.shift(1) > mean\n");
    expect(findingsFor(result, "QL-BIAS-001")).toHaveLength(0);
  });

  it("flags future-indexed iloc access", async () => {
    const result = await audit("import pandas as pd\nfor i in range(n):\n    x = df.iloc[i + 1]\n");
...
describe("QL-BIAS-002 centered window", () => {
  it("flags center=True rolling windows", async () => {
  ...
  it("does not flag trailing windows", async () => {
...
describe("QL-COST-001 / QL-COST-002 transaction costs", () => {
  it("flags a backtest constructed without any cost modeling", async () => {
  ...
  it("does not flag when fees are configured", async () => {
    const result = await audit(
  ...
  it("flags explicitly zero fees", async () => {
    const result = await audit(
...
describe("QL-RISK rules", () => {
  it("QL-RISK-001 flags order placement without stop-loss logic", async () => {
    const result = await audit(
  ...
  it("QL-RISK-001 passes when stop handling exists", async () => {
    const result = await audit(
  ...
  it("QL-RISK-004 flags near-full-cash position sizing", async () => {
    const result = await audit(
...
describe("QL-STRUCT-002 bare except", () => {
  it("flags bare except handlers", async () => {
    const result = await audit("try:\n    run()\nexcept:\n    pass\n");
...
describe("category selection contract", () => {
  it("runs no rules when no categories are selected", async () => {
    const result = await audit("import pandas as pd\nsignal = close.shift(-1) > mean\n", []);
  ...
  it("only runs rules from the selected categories", async () => {
    const result = await audit("import pandas as pd\nsignal = close.shift(-1) > mean\n", [
      "Risk Management",
    ]);
    expect(findingsFor(result, "QL-BIAS-001")).toHaveLength(0);
  ...
  it("registry metadata is complete for every rule", async () => {
