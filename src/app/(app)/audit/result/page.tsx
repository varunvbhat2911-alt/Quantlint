...
/* Evidence provenance (7C/7L) — deterministic engine classification. */
function EvidenceBadge({ kind }: { kind?: string }) {
  if (!kind) return null;
  const config: Record<string, { label: string; className: string; note: string }> = {
    direct: {
      label: "Direct evidence",
      className: "bg-emerald-500/8 text-emerald-600 dark:text-emerald-400",
      note: "Matched directly in the submitted source.",
    },
    inferred: {
      label: "Inferred",
      className: "bg-blue-500/8 text-blue-600 dark:text-blue-400",
      note: "Derived from structural analysis of the submitted source.",
    },
    absence: {
      label: "Absence-based check",
      className: "bg-amber-500/8 text-amber-600 dark:text-amber-400",
      note: "An expected safeguard was not detected in the submitted source. Absence is not proof of behavior — the check only means no matching pattern was found.",
    },
  };
  const c = config[kind];
  if (!c) return null;
...
/* Inline AI explanation block — interpretive layer, visually distinct from
 * the deterministic rule result above it (7L). */
function InlineAIExplanation({ ai }: { ai: NonNullable<Violation["aiExplanation"]> }) {
  return (
    <div className="rounded-lg border border-violet-500/25 bg-violet-500/[0.04] p-3.5 space-y-2.5">
      <div className="flex items-center justify-between gap-2 flex-wrap">
        <div className="flex items-center gap-1.5">
          <Sparkles className="h-3.5 w-3.5 text-violet-500 dark:text-violet-400" />
          <span className="text-[10px] font-mono uppercase tracking-wider text-violet-600 dark:text-violet-400">
            AI Explanation — interpretive, not a rule result
          </span>
        </div>
        <div className="flex items-center gap-1.5">
          {ai.evidenceLevel && (
            <span className="text-[10px] font-mono text-muted-foreground">
              evidence: {ai.evidenceLevel}
            </span>
          )}
          <span className="text-[10px] font-mono text-muted-foreground">
      ...
      <p className="text-xs text-muted-foreground leading-relaxed">{ai.explanation}</p>
      {ai.whyItMatters && (
      ...
      {ai.suggestedFix && (
        <p className="text-xs text-muted-foreground leading-relaxed">
          <span className="font-medium text-foreground/70">AI-suggested fix: </span>
          {ai.suggestedFix}
        </p>
      )}
      {ai.caveats && ai.caveats.length > 0 && (
        <div className="space-y-1">
          {ai.caveats.map((caveat, i) => (
            <p key={i} className="text-[11px] text-muted-foreground/80 leading-relaxed">
              <span className="font-mono">⚠ caveat: </span>
              {caveat}
            </p>
          ))}
        </div>
      ...
      <p className="text-[10px] text-muted-foreground/50 font-mono">
        Generated by {ai.model ?? "the configured AI model"} — verify against the
        deterministic finding above.
      </p>
    </div>
  );
}
      ...
      >
        <div className="overflow-hidden">
          <div className="px-4 pb-4 pt-1 space-y-4 border-t border-border/30">
            ...
            {v.evidence === "absence" && (
              <p className="text-xs text-muted-foreground/80 leading-relaxed border border-amber-500/20 bg-amber-500/[0.04] rounded-md px-3 py-2">
                This is an absence-based check: the safeguard described above
                was not detected in the submitted source. That is not proof of
                incorrect behavior — only that no matching pattern was found by
                static analysis.
              </p>
            )}
            {v.fixSnippet && (
            ...
            {/* AI explanation — interpretive layer, distinct by design */}
            {v.aiExplanation && <InlineAIExplanation ai={v.aiExplanation} />}
          </div>
        </div>
      </div>
    </div>
...
function AIExplanationTab({
  ...
  if (explanations.length === 0) {
    return (
      <EmptyState
        icon={Sparkles}
        title="No AI explanations for this audit"
        description={
          aiUnavailable
            ? "AI enrichment was unavailable for this audit. The deterministic findings above are complete and unaffected."
            : "AI explanations appear here when enrichment succeeds for a finding. Findings without an explanation were either informational, capped by cost limits, or failed validation."
        }
      />
    );
  }
  ...
  return (
    <div className="space-y-4">
      {/* Disclaimer */}
      <div className="flex items-start gap-2 rounded-lg border border-violet-500/25 bg-violet-500/[0.04] p-3">
        <Sparkles className="h-3.5 w-3.5 text-violet-500 dark:text-violet-400 shrink-0 mt-0.5" />
        <div>
          <p className="text-xs font-medium text-muted-foreground">
            AI-generated explanations — interpretive layer, not rule results
          </p>
          <p className="text-[11px] text-muted-foreground/70 mt-0.5">
            The deterministic engine detected the findings; the AI only
            interprets them from the submitted evidence. Review by a qualified
      ...
      {explanations.map((ex) => (
        <Card key={ex.id} className="border-border/40 bg-card/40">
          ...
          <CardContent className="p-5 pt-0 space-y-4">
            ...
            {ex.caveats && ex.caveats.length > 0 && (
              <div className="space-y-1.5">
          ...
          <div className="min-w-0">
            <Tabs defaultValue="overview">
              ...
              <TabsContent value="ai" className="mt-6">
                <AIExplanationTab
                  explanations={result.aiExplanations}
                  aiUnavailable={result.timeline.some((t) =>
                    /AI enrichment failed|AI enrichment skipped/i.test(t.label),
                  )}
                />
              </TabsContent>
